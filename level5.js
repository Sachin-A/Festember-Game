function level5()
    {
		...
		var winx5 =35 * scale * xlimit / 1200;
		var winy5 = ylimit - (18 * scale * ylimit / 900);
        ...
        function update5()
        {
            ...
            ctx.beginPath();
            ctx.arc(winx5, winy5, 30 * (xlimit + ylimit) / (1200 + 900), 0, Math.PI * 2, false);
            ctx.closePath();
            ctx.fill();
            if (scope == 1)
                ray();
			if (Math.sqrt(Math.pow(wheel.GetPosition().x * scale - winx5, 2) + Math.pow(wheel.GetPosition().y * scale - winy5, 2)) > 50)
            {
